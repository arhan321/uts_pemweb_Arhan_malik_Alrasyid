        ...
        <form method="POST" action="{{ route("admin.products.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('mesin') ? 'is-invalid' : '' }}" name="mesin" id="mesin">
                    <option value="" disabled selected>{{ trans('cruds.product.fields.mesin') }}</option>
                    @foreach(App\Models\Product::JENIS_MESIN as $key => $label)
                        <option value="{{ $key }}" {{ old('mesin') === $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('mesin'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cc') ? 'is-invalid' : '' }}" type="number" name="cc" id="cc" value="{{ old('cc', '') }}" step="0.01">
                @if($errors->has('cc'))
                    <div class="invalid-feedback">
                        {{ $errors->first('cc') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pajak') ? 'is-invalid' : '' }}" type="text" name="pajak" id="pajak" value="{{ old('pajak', '') }}">
                @if($errors->has('pajak'))
                    <div class="invalid-feedback">
                        {{ $errors->first('pajak') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.pajak_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="transmition">{{ trans('cruds.product.fields.transmition') }}</label>
                <select class="form-control {{ $errors->has('transmition') ? 'is-invalid' : '' }}" name="transmition" id="transmition">
                    <option value="" disabled selected>{{ trans('cruds.product.fields.transmition') }}</option>
                    @foreach(App\Models\Product::TRANSMITION as $key => $label)
                        <option value="{{ $key }}" {{ old('transmition') === $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
